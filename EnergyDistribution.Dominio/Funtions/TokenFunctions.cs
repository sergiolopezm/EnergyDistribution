...
namespace EnergyDistribution.Domain.Services
{
    public partial class TokenRepository
    {
        ...
        private string CrearTokenUsuario(Usuario usuario, string ip)
        {
            ...
            claims.AddClaim(new Claim("Random", random.Next(100000000, 1000000000) + ""));
            ...
            claims.AddClaim(new Claim("Rol", usuario.Rol!));
            claims.AddClaim(new Claim("Ip", ip));

            var credencialesToken = new SigningCredentials(new SymmetricSecurityKey(_keyBytes), SecurityAlgorithms.HmacSha256Signature);
